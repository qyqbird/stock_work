...
class StockSelect(object):
    ...
    def concept_analysis(self,concept_code):
        ...
        raw_data = raw_data.sort_index(by=['totals'])
        raw_data = raw_data.sort_index(by=['reservedPerShare'], ascending=False)
    ...
    #打印股票详细信息
    @staticmethod
    def print_stock_info(code,row_info, gross_profit_rate):
        '''
        code 是股票代码
        gross_profit_rate 是毛利率
        虽然炒概念，也要炒概念中股票比较优质的资产
        '''
        print "stock:{0} name:{1} 公积金:{2} 股本:{3:.2f}亿 PE:{4} ROE:{5:.3f} 毛利率:{6} 行业:{7}".format(code,
                row_info['name'],row_info['reservedPerShare'],row_info['totals']/10000, 
                row_info['pe'],row_info['earn_ratio'],gross_profit_rate,row_info['industry'])   
    #资本公积金，股本选股，用于高转送行情
    def capital_reserve(self,rePerShare=3, total_share=80000, eps=0.5):
        '''
        '''
        result = self.foundmental_data
        # 1. 保留公积金 >2 元
